...
const int ATRIUM_REST = 90;
const int ATRIUM_BEAT = 55;

const int VENTRICLE_REST = 90;
const int VENTRICLE_BEAT = 125;
...
unsigned long lastDisplayUpdate = 0;bool atriumContracting = false;
...
void computeBPMFromWindow() {
	...
	int lastPeakIndex =
		...
		portENTER_CRITICAL(
		    ...
		    millis();		portEXIT_CRITICAL(
		    &bpmMutex
		);		return;

	}	long intervalSum = 0;	for (
	    int i = 1;
	    i < peakCount;
	    i++
	) {
	    ...
	    SAMPLE_INTERVAL_MS;	int computedBPM =
...
void playNote(
	...
	portENTER_CRITICAL(
	    ...
	    audioSampleCounter;	notePlaying =
...
void setup() {
	...
	Serial.begin(
	    ...
	    OLED_SDA,
	    OLED_SCL
	);	loadSettings();	atriumServo.attach(
	    SERVO_ATRIUM_PIN
