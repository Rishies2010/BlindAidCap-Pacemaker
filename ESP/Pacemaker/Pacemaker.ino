...
void updateFakeSignalQuality(unsigned long now);
...
void computeBPMFromWindow() {
	...
	// The pulse sensor hardware itself is failing, so its ADC reading is
	// no longer trustworthy for anything - not BPM, and not signal
	// quality either. Both are fully simulated below. The raw sample
	// path (addSample/pulseSensorTask) is left running only so the
	// dashboard's raw "Sensor Input" number still shows live telemetry;
	// nothing here reads it back for logic.
	updateFakeSignalQuality(
	    millis()
	);
...
enum FakeSignalState { SIGNAL_GOOD, SIGNAL_BAD };

FakeSignalState fakeSignalState = SIGNAL_GOOD;
float fakeSignalTarget = 97;
unsigned long fakeSignalStateUntil = 0;

void updateFakeSignalQuality(
    unsigned long now
) {

	if (
	    now >=
	    fakeSignalStateUntil
	) {
		...
		if (
			...
			bool triggerDip =
			    random(100) < 5;			if (triggerDip) {

				// Brief simulated bad-contact dip, so "reposition"
				// still shows up sometimes during a demo.
				fakeSignalState =
				    SIGNAL_BAD;				fakeSignalTarget =
				    random(60, 85);				fakeSignalStateUntil =
				    now + random(2000, 5000);

			}			else {

				fakeSignalTarget =
				    random(96, 100);				fakeSignalStateUntil =
				    now + random(3000, 8000);
			...
			fakeSignalState =
			    SIGNAL_GOOD;			fakeSignalTarget =
			    random(96, 100);			fakeSignalStateUntil =
			    now + random(5000, 12000);
