...
const led1Status = $("#led1Status");

const led2Status = $("#led2Status");

const liveBadgeText = $("#liveBadgeText");

const bpmMiniText = $("#bpmMiniText");
...
function updateDashboard(data) {
  ...
  updateHardwareStatus(led1Status, data.atriumActive);

  updateHardwareStatus(led2Status, data.atriumActive);

  if (liveBadgeText) {
    liveBadgeText.textContent = data.manualBPM ? "TEST" : "LIVE";
  }

  if (bpmMiniText) {
    bpmMiniText.textContent = data.manualBPM
...
async function loadSettings() {
  try {
    ...
    $("#atriumRestInput").value = data.atriumRest;

    $("#atriumBeatInput").value = data.atriumBeat;

    $("#ventricleRestInput").value = data.ventricleRest;

    $("#ventricleBeatInput").value = data.ventricleBeat;
  ...
  try {
    const manual = await api("/api/manual-bpm");

    $("#manualBPMToggle").checked = manual.enabled;

    $("#manualBPMInput").value = manual.bpm;
  } catch (error) {
    console.error(error);
  }
}
...
async function saveSettings() {
  const settings = {
    ...
    atriumRest: Number($("#atriumRestInput").value),

    atriumBeat: Number($("#atriumBeatInput").value),

    ventricleRest: Number($("#ventricleRestInput").value),

    ventricleBeat: Number($("#ventricleBeatInput").value),
  };
...
async function applyManualBPM() {
  const payload = {
    enabled: $("#manualBPMToggle").checked,

    bpm: Number($("#manualBPMInput").value),
  };

  const button = $("#applyManualBPM");
  ...
  try {
    ...
    alert("Unable to update manual BPM");
  } finally {
    if (button) {
...
$("#manualBPMToggle").addEventListener("change", applyManualBPM);

$("#applyManualBPM").addEventListener("click", applyManualBPM);
...
const searchItems = [
...

  {
    ...
    description: "Atrium and ventricle rest/beat angles",

    action: "settings",
  },

  {
    title: "Manual BPM Testing",

    description: "Override the pulse reading for testing",
