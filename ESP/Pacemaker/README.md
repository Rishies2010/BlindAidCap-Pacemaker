...
This document covers the full electrical build for the pacemaker demonstration model. The ESP32 reads a pulse signal, classifies it against a user-configurable normal range, and when the reading is abnormal, drives a dual-chamber servo/LED model at a corrected rate toward a target BPM. Two additional indicator LEDs flash in sync with the atrium chamber for a more visible heartbeat cue from across the room. It also plays a synthesized lub-dub heartbeat through a small amplifier and speaker, sounds an alert buzzer when pacing engages, renders status on a 128×64 OLED, and broadcasts its own WiFi access point serving a live web dashboard.

Settings (volume, target BPM, normal range, servo rest/beat angles, theme, accent) and the event log persist across reboots in the ESP32's NVS flash.
...
| **Breadboard B — "Actuation board"** | Hosts the 5V/GND rails, both servos, all four LEDs (atrium, ventricle, and two synced indicator LEDs), buzzer, speaker amp. |
...
| Indicator LED 1 | GPIO4 | Through 220Ω resistor. Flashes in sync with the atrium LED |
| Indicator LED 2 | GPIO32 | Through 220Ω resistor. Flashes in sync with the atrium LED |
...
> **Why the audio goes on GPIO25, not GPIO13:** the firmware generates the heartbeat as a real analog waveform using the ESP32's internal 8-bit DAC. Only GPIO25 (DAC1) and GPIO26 (DAC2) have this hardware. GPIO13 has no DAC, so an amp wired to GPIO13 will be silent under the current firmware. This is also why the two servos had to move off GPIO25/26 to GPIO18/19 — those pins are now reserved for analog output. It's also why Indicator LED 1 sits on GPIO4 rather than GPIO26: GPIO26 stays fully unused so nothing can load down the DAC pair.

> **Pins to leave alone:** GPIO25 and GPIO26 are the only DAC pins. Do not attach servos, LEDs, or anything else to them once the amp is wired to GPIO25 — including the two indicator LEDs, which is why they're routed to GPIO4 and GPIO32 instead.
...
> The rest and beat angles for this servo are set in firmware defaults (90° rest / 70° beat) but are adjustable per-chamber from the web dashboard's Settings panel and persist across reboots — use this to fine-tune the mechanical throw once the servo horn/linkage is attached, rather than re-flashing.
...
> Same rest/beat angle note as the atrium servo above (90° rest / 70° beat defaults, adjustable from the dashboard).
...
### 5.7 Indicator LED 1 + 220Ω Resistor (Breadboard B)
1. Resistor: one leg → ESP32 GPIO4 (direct jumper), other leg → LED long leg (anode).
2. LED short leg (cathode) → Breadboard B (−) rail.

> This LED and Indicator LED 2 below both flash whenever the atrium LED does — they don't have independent logic, they're purely a brighter/more visible echo of the atrium beat for demo visibility. Wire them the same way as the atrium/ventricle LEDs.

### 5.8 Indicator LED 2 + 220Ω Resistor (Breadboard B)
1. Resistor: one leg → ESP32 GPIO32 (direct jumper), other leg → LED long leg (anode).
2. LED short leg (cathode) → Breadboard B (−) rail.

### 5.9 Buzzer (Breadboard B)
| Buzzer pin | Connects to |
|---|---|
| + | ESP32 GPIO14 (direct jumper) |
| − | Breadboard B (−) rail |

### 5.10 Speaker Amp — CA-8403 / PAM8403 (Breadboard B)
| Amp pin | Connects to |
|---|---|
...
5. **GPIO25 is wired only to the amplifier's INL, and GPIO26 is wired to nothing at all.** No servo, LED, or sensor should share either pin. GPIO18 and GPIO19 are wired only to the two servo signal lines; GPIO4 and GPIO32 are wired only to Indicator LEDs 1 and 2.
...
**Settings persistence:** volume, target BPM, normal range, per-chamber servo rest/beat angles, theme, and accent are saved to NVS via `Preferences` and survive reboot and re-flash. To reset them, either clear NVS from the Arduino IDE (`Tools → Erase All Flash Before Sketch Upload`) or POST to `/api/restart` after clearing them from the dashboard.
...
| Indicator LEDs 1/2 stay dark while the atrium LED blinks | Check their resistor/orientation the same way as the atrium/ventricle LEDs — they're wired independently even though they mirror the atrium timing |
